...
class accel_scoreboard extends uvm_scoreboard;
    ...
    function void write_axil(axil_csr_tnx t);
        ...
        if(t.we == 1) begin
            ...
            // Skip performance counter reads — monitoring only
    ...
    function void write_result(axis_result_tnx t);
        ...
        // Debug — check what scoreboard received
       `uvm_info("SCO", $sformatf("Computing: w0[0]=%0d w1[0]=%0d a0=%0d a1=%0d",w.w0[0], w.w1[0], a.a0, a.a1), UVM_MEDIUM)
        ...
        for(int c = 0; c < TB_COLS; c++) begin
            if(t.result[c] !== expected[c]) begin
                ...
                pass_count++;
                //`uvm_info("SCO",$sformatf("PASS col=%0d val=0x%0h",c, t.result[c]), UVM_MEDIUM)
            end
        end
        ...
        for(int c = 0; c < TB_COLS; c++) begin
            ...
            for(int r = 0; r < TB_ROWS; r++) begin
                ...
                if(a.mode_dense) begin
                    // Dense mode — ignores idx completely
                    // Phase 0: always uses a0, a1
                    // Phase 1: always uses a2, a3
                    expected[c] += w.w0[r*TB_COLS+c] * act[0]; // a0
                    expected[c] += w.w1[r*TB_COLS+c] * act[1]; // a1
                    expected[c] += w.w0[r*TB_COLS+c] * act[2]; // a2
                    expected[c] += w.w1[r*TB_COLS+c] * act[3]; // a3
                end
                else begin
                    // Sparse mode — uses idx0, idx1 from weights
                    automatic int i0, i1;
                    i0 = w.idx0[r*TB_COLS+c];
                    i1 = w.idx1[r*TB_COLS+c];
