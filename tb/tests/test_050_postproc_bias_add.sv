`ifndef TEST_050_POSTPROC_BIAS_ADD_SV
`define TEST_050_POSTPROC_BIAS_ADD_SV

class test_050_postproc_bias_add extends base_test;
  `uvm_component_utils(test_050_postproc_bias_add)

  function new(string name = "test_050_postproc_bias_add", uvm_component parent = null);
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    $display("=============================Test_050 : Post-Proc Bias Addition Per-Column - Start=======================================");
    ...
    // Build weight tile (ALL ZERO)
    for (int i = 0; i < TB_ROWS * TB_COLS; i++) begin
      w_tile_tnx.w0[i] = 0; w_tile_tnx.w1[i] = 0;
    ...
    // Step 2 : Configure post‑proc: bias‑add only, bias[c]=50
    $display("                Step 2 : Configure PP — PP_BIAS_ADD, load bias=50 for all 16 cols                ");
    wr_seq.csr_addr = CSR_PP_CTRL;  
    wr_seq.csr_data = 32'h0000_0001;  // PP_BIAS_ADD
    wr_seq.start(env.axil_a.seqr);
    ...
    // Load bias = 50 for all 16 columns via auto‑increment interface
    ...
    for (int c = 0; c < 16; c++) begin
      ...
      wr_seq.csr_data = 32'd50;
      wr_seq.start(env.axil_a.seqr);
    end

    // Step 3 : Configure compute — all‑zero weights, 1 vector, dense
    $display("                Step 3 : Configure compute — all‑zero weights, 1 vector, dense                ");
    ...
    // Step 4 : Stream all‑zero weight tile
    $display("                Step 4 : Stream all‑zero weight tile                ");
    ...
    $display("                Step 6 : Stream 1 activation vector                 ");
    a_stream_seq.t = a_stream_tnx;
    a_stream_seq.start(env.act_a.seqr);

    // Wait for done (just in case)
    @(posedge vif.clk iff probe_if.done === 1'b1);
    repeat(10) @(posedge vif.clk);

    $display("===================================================Test_050 : End of the test=================================================================");
